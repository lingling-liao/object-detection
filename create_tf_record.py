...
def dict_to_tf_example(data,
    ...
    Args:
      data: Dict holding XML fields for a single image (obtained by running
...
def main(_):
    ...
    writer = tf.python_io.TFRecordWriter(
        os.path.join(FLAGS.data_dir, FLAGS.set+'.record'))
    
    label_map_path = os.path.join(FLAGS.data_dir, 'label_map.pbtxt')
    label_map_dict = label_map_util.get_label_map_dict(label_map_path)
    
    examples_path = os.path.join(FLAGS.data_dir, 'sets', FLAGS.set+'.txt')
    examples_list = dataset_util.read_examples_list(examples_path)
    
    annotations_dir = os.path.join(FLAGS.data_dir, 'annotations')
